...
type KeypairInitParameters struct {
	...
	// A unique identifier for the Key Pair record.
	PairName *string `json:"pairName,omitempty" tf:"pair_name,omitempty"`
...
type Keypair struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.pairName) || (has(self.initProvider) && has(self.initProvider.pairName))",message="spec.forProvider.pairName is a required parameter"
