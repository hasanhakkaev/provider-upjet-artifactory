...
type CargoRepositoryInitParameters struct {
	...
	// This is the index url, expected to be a git repository. Default value in UI is "https://github.com/rust-lang/crates.io-index"
...
// CargoRepository is the Schema for the CargoRepositorys API.
...
type CargoRepository struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.gitRegistryUrl) || (has(self.initProvider) && has(self.initProvider.gitRegistryUrl))",message="spec.forProvider.gitRegistryUrl is a required parameter"
...
func init() {
	SchemeBuilder.Register(&CargoRepository{}, &CargoRepositoryList{})
}
